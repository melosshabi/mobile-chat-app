          ...
          <Pressable onLongPress={() => {
            ...
            <View style={[styles.message, messageDoc.senderID === auth.currentUser?.uid ? styles.loggedUserMessage : {}]}>
              {/* The pressable which navigates to the profile of the user whose pfp got tapped on */}
                <Pressable onPress={() => handleProfileNavigation(messageDoc.senderID, messageDoc.senderProfilePicture, messageDoc.senderName)} style={[styles.profilePictureWrapper]}>
                  <Image style={[styles.messageProfilePicture]} source={messageDoc.senderProfilePicture ? {uri:messageDoc.senderProfilePicture} : require('../images/user.png')}/>
                </Pressable>
                    {/* Message Text */}  
